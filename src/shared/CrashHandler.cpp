...
{
	sLog.outString( "Crash Handler: Advanced crash handler initialized." );

	try
	{
		sLog.outString( "sql: Waiting for all database queries to finish..." );
		
		WorldDatabase.ThreadEnd();
		CharacterDatabase.ThreadEnd();
		sLog.outString( "sql: All pending database operations cleared.\n" );
//		sWorld.SaveAllPlayers();
		sLog.outString( "sql: Data saved." );
	}
	catch(...)
	{
		sLog.outString( "sql: Threw an exception while attempting to save all data." );
	}

	sLog.outString( "Server: Closing." );
